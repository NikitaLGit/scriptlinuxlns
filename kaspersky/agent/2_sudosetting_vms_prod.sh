...
cat /tmp/sudoers_final >> /etc/sudoers

# Проверяем синтаксис файла sudoers
visudo -c

if [ $? -eq 0 ]; then
  echo "Изменения в файле /etc/sudoers успешно применены"
else
...
echo "Проверьте в новом файле /etc/sudoers, чтобы не было окружение PATH в директиве Default env_keep!"
